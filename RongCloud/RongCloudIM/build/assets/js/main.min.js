...
account.controller("signinController", ["$scope", "$state", "mainServer", "mainDataServer", "conversationServer", "RongIMSDKServer",
    function ($scope, $state, mainServer, mainDataServer, conversationServer, RongIMSDKServer) {
        ...
        webimutil.CookieHelper.removeCookie("loginuserid");
        webimutil.CookieHelper.removeCookie("loginusertoken");
        var userId = localStorage.getItem("RongIMUserId");
        //var token = localStorage.getItem("RongIMToken");
        if (userId != null) {
            //mainDataServer.loginUser.id = userId;
            //mainDataServer.loginUser.token = token;
            //var exdate = new Date();
            //webimutil.CookieHelper.setCookie("loginuserid", userId, exdate.toGMTString());
            //webimutil.CookieHelper.setCookie("loginusertoken", token, exdate.toGMTString());
            //$state.go("main");
            $scope.user.accountNumber = userId;
            $scope.user.passWord = "123";
            $scope.signin();
        } else {
            location.href = "/RongCloud/Index";
        }
...
var UPLOADDOMAIN = "//upload.qiniu.com";
var UPLOADDOMAIN_BASE64 = "//upload.qiniu.com/putb64/-1";
...
conversationCtr.controller("conversationController", ["$scope", "$state", "mainDataServer", "conversationServer", "mainServer", "RongIMSDKServer", "$http", "$timeout", "$location", "$anchorScroll",
    function ($scope, $state, mainDataServer, conversationServer, mainServer, RongIMSDKServer, $http, $timeout, $location, $anchorScroll) {
        ...
        var messageItem = {
            4: function (file) {
                var name = file.name || '', index = name.lastIndexOf('.') + 1, type = name.substring(index);
        ...
        var urlItem = {
            4: function (data, callback) {
                RongIMSDKServer.getFileUrl(4, data.filename, data.name).then(function (result) {
            ...
            1: function (data, callback) {
                RongIMSDKServer.getFileUrl(1, data.filename, null).then(function (result) {
                    data.downloadUrl = result.downloadUrl;
                    var message = createMessage(data);
                    callback(message);
                }, function (error) {
        ...
        var getFileUrl = function (data, callback) {
            urlItem[data.fileType](data, callback);
        };
        var callbacks = {
            onError: function (error, type, messageId) {
                uploadError(type, messageId, error);
            },
            onProgress: function (loaded, total, type, messageId) {
            ...
            onCompleted: function (data, type, messageId) {
                data.fileType = type;
                webimutil.Helper.compressImage(data.thumbnail, function (compressBase64) {
                    data.thumbnail = compressBase64;
                    getFileUrl(data, function (message) {
                        RongIMSDKServer.sendMessage($scope.currentConversation.targetType, $scope.currentConversation.targetId, message).then(function (msg) {
                            if (type == 1) {
        ...
        var fileNode = document.getElementById('upload-file');
        fileNode.onchange = function () {
            ...
            var config = {
                ...
                getToken: function (callback) {
                    RongIMSDKServer.getFileToken(this.fileType).then(function (data) {
                        callback(data.token);
                    }, function (error) {
            ...
            UploadClient.initFile(config, function (uploadFile) {
                fileNode.value = '';
                $scope.uploads[messageId] = uploadFile;
                uploadFile.upload(file, {
                    onError: function (error) {
                        callbacks.onError(error, config.fileType, messageId);
                    },
                    onProgress: function (loaded, total) {
                        callbacks.onProgress(loaded, total, config.fileType, messageId);
                    },
                    onCompleted: function (data) {
                        callbacks.onCompleted(data, config.fileType, messageId);
                    }
                });
            });
        };
        var imageNode = document.getElementById('upload-image');
        imageNode.onchange = function () {
            ...
            UploadClient.initImage(config, function (uploadFile) {
                ...
                uploadFile.upload(file, {
                    onError: function (error) {
                        callbacks.onError(error, config.fileType);
                    },
                    onProgress: function (loaded, total) {
                        callbacks.onProgress(loaded, total, config.fileType);
                    },
                    onCompleted: function (data) {
                        callbacks.onCompleted(data, config.fileType);
                    }
                });
            });
        };
        function uploadError(type, messageId, err) {
            if (type == 1) {
                $scope.uploadStatus.show = false;
        ...
        function uploadBase64(strBase64, file) {
            ...
            UploadClient.initImgBase64(imgConfig, function (uploadFile) {
                uploadFile.upload(strBase64, {
                    onError: function (error) {
                        callbacks.onError(error, imgConfig.fileType);
                    },
                    onProgress: function (loaded, total) {
                        callbacks.onProgress(loaded, total, imgConfig.fileType);
                    },
                    onCompleted: function (result) {
                        showLoading(false);
                        $scope.showPasteDiv(false);
                        callbacks.onCompleted(result, imgConfig.fileType);
                    }
                });
            });
        }
...
(function (webimutil) {
    ...
    var Helper = (function () {
        ...
        Helper.showBigImage = function (url) {
        ...
        Helper.compressImage = function (base64, callback) {
            var maxLength = 55490;
            if (!base64) {
...
(function (webimmodel) {
    ...
    var Message = (function (_super) {
        ...
        Message.messageToNotification = function (msg, operatorid, isnotification) {
            ...
            if (msgtype == webimmodel.MessageType.ImageMessage || msg.objectName == 'RC:GIFMsg') {
                msgContent = "[图片]";
            }
...
friendinfo.controller("friendinfoController", ["$scope", "$rootScope", "$state", "$stateParams", "$window", "mainDataServer", "mainServer",
    function ($scope, $rootScope, $state, $stateParams, $window, mainDataServer, mainServer) {
        $scope.$on("$viewContentLoaded", function () {
            setPortrait();
        });
        function setPortrait() {
...
groupInfo.controller("groupinfoController", ["$scope", "$rootScope", "$state", "$stateParams", "mainDataServer", "mainServer", "RongIMSDKServer",
    function ($scope, $rootScope, $state, $stateParams, mainDataServer, mainServer, RongIMSDKServer) {
        ...
        $scope.kickMember = function () {
            var membersid = [];
            $scope.groupInfo.memberList.filter(function (item) {
                if (item.isSelected) {
...
groupInfo.directive("member", ["$state", "mainDataServer", function ($state, mainDataServer) {
    return {
        ...
        link: function (scope, ele, attr) {
            ...
            scope.loginUserid = mainDataServer.loginUser.id;
            scope.$watch("item.isSelected", function (newValue, oldValue) {
                if (newValue == oldValue) {
                    return;
                }
            });
...
mainServer.factory("mainDataServer", ["$q", "RongIMSDKServer", "mainServer", function ($q, RongIMSDKServer, mainServer) {
        ...
        updateConversations: function () {
            ...
            RongIMSDKServer.getConversationList().then(function (list) {
                ...
                for (var i = 0, length = list.length; i < length; i++) {
                    ...
                    if (list[i].conversationType == RongIMLib.ConversationType.CUSTOMER_SERVICE || list[i].conversationType == RongIMLib.ConversationType.DISCUSSION || list[i].conversationType == RongIMLib.ConversationType.SYSTEM || list[i].conversationType == RongIMLib.ConversationType.CHATROOM)
                        continue;
    ...
    var contactsList = {
        ...
        updateGroupNameById: function (id, name) {
            for (var i = 0; i < this.groupList.length; i++) {
                var item = this.groupList[i];
                if (item.id == id) {
                    item.name = name;
...
mainServer.factory("RongIMSDKServer", ["$q", function ($q) {
    var RongIMSDKServer = {};
    ...
    RongIMSDKServer.getFileToken = function (type) {
        var defer = $q.defer();
    ...
    RongIMSDKServer.getFileUrl = function (type, hash, name) {
        var defer = $q.defer();
    ...
    function ($scope, $state, $window, $timeout, $http, mainDataServer, conversationServer, mainServer, RongIMSDKServer, appconfig) {
        ...
        if (!mainDataServer.loginUser.id) {
            ...
            if (userid) {
                ...
                mainServer.user.logout().success(function () {
                    webimutil.CookieHelper.removeCookie("loginuserid");
                    window.localStorage.removeItem("RongIMUserId");
                    window.localStorage.removeItem("RongIMToken");
                    mainDataServer.loginUser = new webimmodel.UserInfo();
            ...
            for (var i = 0, len = groups.length; i < len; i++) {
                ...
                !function (groupid) {
                    mainServer.group.getGroupMember(group.id).success(function (rep) {
                        var groupInfo = mainDataServer.contactsList.getGroupById(groupid);
                        ...
                        for (var j = 0, len = members.length; j < len; j++) {
                            ...
                            if (j == 0 && groupInfo) {
                                groupInfo.creater = members[j].user.id;
                            }
                        }
                    });
                }(group.id);
        ...
        RongIMSDKServer.setOnReceiveMessageListener({
            onReceived: function (data) {
                ...
                var msg = webimmodel.Message.formatGIFMsg(data);
                msg = webimmodel.Message.convertMsg(msg);
                if (msg.targetId == "") {
...
mainDire.directive("groupitem", ["$state", function ($state) {
    return {
        restrict: "E",
        scope: { item: "=" },
