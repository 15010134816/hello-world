...
namespace io.rong.methods.group
{
...

    {
        ...
        ///查询群成员
        /// </summary>
        /// <param name="group">群组.Id。（必传）</param>
        /// <returns></returns>
        ...
        /// <param name="group">群组.Id。（必传）</param>
        /// <returns></returns>
        public GroupUserResult GetGroupUser(GroupModel group)
        {
            String errMsg = CommonUtil.CheckFiled(group, PATH, CheckMethod.GET);
            if (null != errMsg)
            {
                return (GroupUserResult)RongJsonUtil.JsonStringToObj<GroupUserResult>(errMsg);
            }
            StringBuilder sb = new StringBuilder();
            sb.Append("&groupId=").Append(HttpUtility.UrlEncode(group.Id.ToString(), UTF8));
            String body = sb.ToString();
            ...
            String result = RongHttpClient.ExecutePost(appKey, appSecret, body,
                            rongCloud.ApiHostType.Type + "/group/user/query.json", "application/x-www-form-urlencoded");
            var model=(GroupUserResult)RongJsonUtil.JsonStringToObj<GroupUserResult>(result);
